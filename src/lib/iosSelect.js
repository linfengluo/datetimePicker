...
(function () {
  ...
  function Layer(html, opts) {
    ...
    const isMobile = window.navigator.userAgent.match(/(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/ig)
    layer_el.className = 'layer' + (isMobile ? '' : ' ispc');
    this.el = el;
  ...
  IosSelect.prototype = {
    ...
    initLayer: function () {
      ...
      var closeText = this.options.closeText || this.options.closeText == '' ? this.options.closeText : '取消';
      ...
      const length = this.options.showSymbol ? this.options.symbols.length : 0
  
      if (this.options.showSymbol) {
        for (let i = 0; i < length; i++) {
          all_html = [...all_html, ...[ '<div class="dirver ' + ( this.level == 3 && i == 2? 'padding' : '')  +'" >', this.options.symbols[i], '</div>'], ...levelHtml[i]]
        }
      }
      
      for (let j = 0; j < 5 - length; j++ ) {
        all_html = [...all_html, ...levelHtml[j + length]]
      }
      
      all_html = [...all_html, ...endHtml].join('\r\n')
    ...
    saveLevelData: function (index, data) {
    ...
    renderLevel: function (index, oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, sixLevelId, levelData) {
      ...
      this.rowData[index] = levelData
      var hasAtId = levelData.some(function (v, i, o) {
        return v.id == curLevelId;
      });
      if (!hasAtId) {
        
        curLevelId = levelData.length > 0 ? levelData[0]['id'] : '';
      }
      var tmpHtml = '';
